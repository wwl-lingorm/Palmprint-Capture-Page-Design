...
class CameraLayout(FloatLayout):
    ...
    def handle_capture(self, instance):
        """处理采集按钮点击"""
        if self.buttons_visible and not self.is_capturing:
    ...
    def reset_capture(self, dt):
        ...
        self.button_container.opacity = 0  # 隐藏按钮

    def on_image_touch(self, instance, touch):
        """处理图像区域触摸事件"""
        # 检查是否点击了触发区域(右上角5%)
        if not self.is_capturing:  # 只在非采集状态下响应触发区域点击
    ...
    def show_buttons(self):
        ...
        if not self.is_capturing:  # 只在非采集状态下自动隐藏
            Clock.schedule_once(lambda dt: self.hide_buttons(), 3)

    def hide_buttons(self):
        """隐藏操作按钮"""
        if not self.is_capturing:  # 只在非采集状态下隐藏
