...
class CameraLayout(FloatLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self.settings_button = Button(
            ...
            background_normal='',
            background_color=[1, 0, 0, 0.3],  # 半透明红色
            text='setting',
            font_size=24
        )
        self.settings_button.bind(on_press=self.show_action_buttons)
        ...
        self.action_buttons = BoxLayout(
            ...
            size_hint=(None, None),
            size=(150, 100),
            pos_hint={'right': 0.98, 'top': 0.88},  # 显示在设置按钮下方
            spacing=5,
            opacity=0
        )
        ...
        self.capture_button.bind(on_press=self.handle_capture)
        self.action_buttons.add_widget(self.capture_button)
        ...
        self.recognize_button.bind(on_press=self.handle_recognize)
        self.action_buttons.add_widget(self.recognize_button)
    ...
    def show_action_buttons(self, instance):
        """显示操作按钮(采集/识别)"""
        if not self.is_capturing:  # 非采集状态下才显示
            self.action_buttons.opacity = 1
            # 3秒后自动隐藏
            Clock.schedule_once(lambda dt: self.hide_action_buttons(), 3)

    def hide_action_buttons(self):
        """隐藏操作按钮"""
        if not self.is_capturing:  # 非采集状态下才隐藏
            self.action_buttons.opacity = 0

    def on_touch_down(self, touch):
        """全局触摸事件处理"""
        # 如果点击在操作按钮区域外，则隐藏按钮
        if not self.action_buttons.collide_point(*touch.pos):
    ...
    def start_capture(self, name, id_number):
        ...
        self.action_buttons.opacity = 1  # 保持按钮可见
    ...
    def reset_capture(self, dt):
        ...
        self.action_buttons.opacity = 0  # 隐藏按钮
...
class MainApp(App):
    ...
    def on_stop(self):
        """应用关闭时释放摄像头"""
        if hasattr(self.root, 'capture') and self.root.capture.isOpened():
