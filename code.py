...
class CameraLayout(FloatLayout):
    ...
    def cancel_capture(self, instance=None):
        ...
        self.popup = Popup(title="Confirm Cancellation",
                        content=content,
                        size_hint=(0.6, 0.4))
        self.popup.open()

    def _confirm_cancel(self, instance):
        """确认取消采集后重置空闲状态 清除已采集照片"""
        self.popup.dismiss()
        if os.path.exists("local_images"):
            pattern = f"{self.name}_{self.id_number}_*.png"
            for filename in os.listdir("local_images"):
                if filename.startswith(f"{self.name}_{self.id_number}_"):
        ...
        if ret:
            ...
            if self.capture_count >= 10:
                ...
                if self.hand == "左":
                    ...
                    self.settings_button.disabled = False
                    self.capture_btn.opacity = 0
                    self.cancel_btn.opacity = 0  # Hide cancel button
